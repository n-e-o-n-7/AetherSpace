...
struct ScrollableView<Content: View>: UIViewControllerRepresentable, Equatable {
	...
	// MARK: - Init
	init(
		_ offset: Binding<CGPoint>, animationDuration: Binding<TimeInterval>,
		showsScrollIndicator: Bool = true, axis: Axis = .vertical,
		onScale: ((CGFloat) -> Void)? = nil, disableScroll: Bool = false,
		forceRefresh: Bool = false, stopScrolling: Binding<Bool> = .constant(false),
		...
		self.scrollViewController = UIScrollViewController(
			rootView: self.content(), offset: self.offset, axis: self.axis, onScale: self.onScale)
	}
	...
	func updateUIViewController(
		_ viewController: UIViewControllerType, context: UIViewControllerRepresentableContext<Self>
	) {
		...
		if self.stopScrolling.wrappedValue {
			viewController.scrollView.setContentOffset(
				viewController.scrollView.contentOffset, animated: false)
			return
		}
	...
	private func newContentOffset(_ viewController: UIViewControllerType, newValue: CGPoint)
		-> CGPoint
	{
	...
	private func duration(_ viewController: UIViewControllerType) -> TimeInterval {
		...
		return self.animationDuration.wrappedValue
			* min(max(TimeInterval(percentageMoved), 0.25), 1)
	}
	...
	lazy var scrollView: UIScrollView = {
		...
		if self.onScale != nil {
			scrollView.addGestureRecognizer(
				UIPinchGestureRecognizer(target: self, action: #selector(self.onGesture)))
		}
